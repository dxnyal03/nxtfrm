...
const NXP = (() => {
  ...
  function home() {
    ...
    document.getElementById('homePage').innerHTML=shell(`${header('Today',N.shortDate(state.date),button(esc(state.gym),'cycleGym()',true))}
      <section class="n99-mission nxp-today"><div class="n99-row"><span class="nxp-caption">${finished?'Workout saved':'Your next session'}</span><button class="n99-text" onclick="showSessionSheet()">Change</button></div><h2>${esc(N.label(state.dayType))}</h2><p>${lift?`${list.length} exercises · ${list.reduce((a,e)=>a+Number(e.sets),0)} working sets`:state.dayType==='Rest'?'A planned day to recharge.':state.dayType==='Zone2'?'An easy, conversational effort.':'Your activity and recovery, together.'}</p>${button(title,finished?"switchTab('train')":action)}</section>
      <button class="nxp-calories" onclick="NXT.openCalories()"><span><small>Daily calorie guide</small><strong>${cal?formatNumber(cal)+' <em>kcal</em>':'Set your target'}</strong></span><span>${cal?'Edit':'Set up'} <i aria-hidden="true">↗</i></span></button>
      ${N.adherenceHTML()}
      <section class="n99-card nxp-trend"><div class="n99-row"><h2>Weight trend</h2><button class="n99-text" onclick="switchTab('weight')">View progress ›</button></div><div class="n99-stats">${N.metric('7-day average',s.current.n>=3?s.current.avg.toFixed(1)+'<small> kg</small>':'—',s.current.n>=3?'From '+s.current.n+' weigh-ins':'Needs 3 readings in 7 days')}${N.metric('Weekly change',s.change===null?'—':N.signed(s.change,2)+'<small> kg</small>',s.change===null?'Building a comparison':'Compared with prior 7 days')}</div><div class="nxp-card-foot"><span>${last?'Latest: '+last.weight.toFixed(1)+' kg · '+N.shortDate(last.date):'Your first weigh-in sets the baseline.'}</span><button class="n99-text" onclick="apx95OpenQuickWeight()">＋ Weight</button></div></section>
      <div class="nxp-quick">${button('Log cardio','showCardioSheet()',true)}${button('Check-in','apx96OpenReadiness()',true)}</div>
      <details class="n99-card nxp-disclosure"><summary><span>Your week</span><small>${N.completedWeek()} lifting days logged</small></summary>${N.weekHTML()}<div class="nxp-card-foot"><span>${N.cardioWeek()} / ${Number(settings.zone2WeeklyTarget)||90} cardio min</span><button class="n99-text" onclick="NXT.more('training')">Edit plan ›</button></div></details>
      <button class="nxp-insight ${r.tone}" onclick="NXT.openReview()"><span class="nxp-caption">Weekly review</span><b>${esc(r.title)}</b><span>${esc(r.action)} ›</span></button>`);
  }
  ...
  function training() {
    ...
    const index=list.findIndex(e=>e.name===ex);
    document.getElementById('trainPage').innerHTML=shell(`${header(N.label(state.dayType),state.gym+' · '+N.shortDate(state.date),button('Session','NXP.sessionMenu()',true))}
      <div class="nxp-session-progress"><span>${count} of ${total} working sets</span><button class="n99-text" onclick="NXP.queue()">Exercise ${index+1} of ${list.length} ›</button></div><div class="n99-session-rail"><span style="width:${total?Math.min(100,count/total*100):0}%"></span></div>
      ...
      <aside class="nxp-rest" aria-label="Rest timer"><span><small>Rest timer</small><b id="apx96TimerValue">${apx96RestRemaining()?apx96FormatTimer(apx96RestRemaining()):'READY'}</b></span><div>${button('+30s','apx96AdjustRest(30)',true)}${button('Skip','apx96SkipRest()',true)}</div></aside>
      <section class="nxp-workspace"><div class="n99-row"><span class="nxp-caption">Current exercise</span><button class="n99-text" onclick="showSubstituteSheet()">Swap</button></div><h2>${esc(ex)}</h2><div class="nxp-set-dots" aria-label="${done} working sets logged, ${t.sets} planned">${Array.from({length:t.sets},(_,i)=>`<span class="${i<done?'done':i===done?'next':''}">${i<done?'✓':i+1}</span>`).join('')}<small>${t.reps[0]}–${t.reps[1]} reps</small></div>
      <div class="nxp-previous"><span>Last time${previous?' · '+N.shortDate(previous.date):''}</span><b>${prev?esc(prev.weight)+' kg × '+esc(prev.reps):'First session at this gym'}</b></div>
      ...
      <div class="n99-form-grid nxp-effort"><label>Set type<select id="n99-set-type" onchange="NXP.rememberInput(this)"><option value="working" ${draft['n99-set-type']!=='warmup'?'selected':''}>Working</option><option value="warmup" ${draft['n99-set-type']==='warmup'?'selected':''}>Warm-up</option></select></label><label>Reps left <small>optional</small><select id="n99-rir" onchange="NXP.rememberInput(this)"><option value="">Not recorded</option>${[0,1,2,3,4].map(n=>`<option value="${n}" ${String(draft['n99-rir'])===String(n)?'selected':''}>${n===4?'4+':n}</option>`).join('')}</select></label></div>
      ...
      <details class="nxp-coach"><summary>${esc(cue.label)}</summary><p>${esc(cue.text)}</p></details>
      ${current.length?`<div class="nxp-set-history"><h3>Today’s sets</h3>${current.map((r,i)=>`<button class="nxp-set-row" onclick="NXP.editCurrentSet(${i})"><span>${r.setType==='warmup'?'W':r.setNum}</span><b>${esc(r.weight)} <small>kg</small> × ${esc(r.reps)}</b><span>Edit ›</span></button>`).join('')}</div>`:''}
      <div class="nxp-quick">${button('Undo last set','apx96UndoLastSet()',true)}${button(index===list.length-1?'Finish workout':'Next exercise','NXT.nextExercise()',true)}</div></section>`}
      <details class="n99-card nxp-disclosure"><summary><span>Workout queue</span><small>${list.length} exercises</small></summary>${queueRows()}${button('Manage exercises','apx96OpenQueueManager()',true)}</details>
      ${!finished?button('Finish workout','NXT.finish()',true):''}`);
    setTimeout(apx96TickTimer,0);
  }
  function queueRows() {return template().map((e,i)=>row(e.name,N.done(e.name)+' / '+e.sets,`NXP.chooseExercise(${i})`,e.reps[0]+'–'+e.reps[1]+' reps')).join('');}
  ...
  function progress() {
    applyAppearance();const s=N.trendStats(),waist=N.cleanRows(N.cfg().waist,'cm').at(-1),v=N.ui.view;
    document.getElementById('weightPage').innerHTML=shell(`${header('Progress','Your trend, not a single reading.',button('＋ Weight','apx95OpenQuickWeight()',true))}<div class="n99-progress-tabs">${[['overview','Overview'],['strength','Strength'],['body','Body']].map(([key,title])=>`<button class="${v===key?'active':''}" aria-pressed="${v===key}" onclick="NXT.setView('${key}')">${title}</button>`).join('')}</div>${v==='strength'?N.strengthHTML():v==='body'?N.bodyHTML():`${N.chartHTML()}${N.tdeeCardHTML()}<div class="nxp-progress-metrics">${N.metric('Weekly change',s.change===null?'—':N.signed(s.change)+'<small> kg</small>','Weekly averages')}${N.metric('Latest waist',waist?waist.cm+'<small> cm</small>':'—',waist?N.shortDate(waist.date):'Optional measurement')}</div>${row('Weekly review',N.review().action,'NXT.openReview()',N.review().title)}${row('Weigh-in history',N.weights().length+' days','NXT.openWeightHistory()')}${row('Strength trends','','NXT.setView(\'strength\')')}${row('Waist & body scans','','NXT.setView(\'body\')')}`}`);
  }
  function more() {
  ...
  function history() {
    ...
    const working=(state.logs||[]).filter(r=>r.date?.startsWith(month)&&r.setType!=='warmup');
    const sessions=new Set(working.map(r=>JSON.stringify([r.date,r.gym||'Gym A',r.dayType||'']))).size;
    document.getElementById('historyPage').innerHTML=shell(`${header('History','Your sessions, easy to revisit.')}<div class="nxp-history-total"><strong>${sessions}</strong><span>lifting sessions this month<br><small>${working.length} working sets</small></span></div><div class="n99-progress-tabs nxp-history-filters">${[['all','All'],['strength','Lifting'],['conditioning','Cardio'],['body','Weight']].map(([k,t])=>`<button aria-pressed="${filter===k}" class="${filter===k?'active':''}" onclick="nxt98SetHistoryFilter('${k}')">${t}</button>`).join('')}</div>${apx96CalendarHTML()}<h2 class="nxp-group-title">${dates.length?'Logged days':'No records in this view'}</h2>${dates.map(d=>historyCard(d)).join('')}${!dates.length?'<p>Your saved workouts, activities and weigh-ins appear here. Try another month or filter.</p>':''}`);
  }
  function historyCard(date) {
    ...
    const parts=[logs.length?logs.length+' sets':'',cardio?cardio+' min cardio':'',fb?'Floorball':'',bw?bw.weight+' kg':''].filter(Boolean);
    return `<button class="nxp-history-card" onclick="NXP.historyDay('${date}')"><span class="nxp-caption">${esc(N.shortDate(date))}</span><b>${esc(types.join(' + ')||(cardio?'Cardio':fb?'Floorball':bw?'Weigh-in':'Daily records'))}</b><span>${esc(gyms.join(' · '))}</span><small>${esc(parts.join(' · ')||'View records')} <i aria-hidden="true">↗</i></small></button>`;
  }
  function historyDay(date) {
    ...
    N.modal(N.shortDate(date),`<div class="n99-stats">${N.metric('Working sets',working.length)}${N.metric('Exercises',new Set(working.map(r=>r.exercise)).size)}</div>${span!==null&&span>=0?`<p class="n99-small">Logging span: ${span} min · first to last recorded set, not measured workout duration.</p>`:''}${ui.historyRows.map((r,i)=>`<button class="nxp-history-set" onclick="NXP.editHistorySet(${i})"><span><b>${esc(r.exercise||'Exercise')}</b><small>${esc(r.gym||'Gym A')} · ${r.setType==='warmup'?'Warm-up':'Set '+esc(r.setNum)}</small></span><b>${esc(r.weight)} × ${esc(r.reps)}</b><span>Edit ›</span></button>`).join('')}${button('All day details & activities',`NXP.otherDayDetails('${date}')`,true)}`);
  }
