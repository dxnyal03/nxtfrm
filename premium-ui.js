...
const NXP = (() => {
  ...
  function progress() {
    ...
    document.getElementById('weightPage').innerHTML=shell(`${header('Progress','Your trend, not a single reading.',button('＋ Weight','apx95OpenQuickWeight()',true))}<div class="n99-progress-tabs">${[['overview','Overview'],['strength','Strength'],['body','Body']].map(([key,title])=>`<button class="${v===key?'active':''}" aria-pressed="${v===key}" onclick="NXT.setView('${key}')">${title}</button>`).join('')}</div>${v==='strength'?N.strengthHTML():v==='body'?N.bodyHTML():`${N.chartHTML()}<div class="nxp-progress-metrics">${N.metric('Weekly change',s.change===null?'—':N.signed(s.change)+'<small> kg</small>','Weekly averages')}${N.metric('Latest waist',waist?waist.cm+'<small> cm</small>':'—',waist?N.shortDate(waist.date):'Optional measurement')}</div>${row('Weekly review',N.review().action,'NXT.openReview()',N.review().title)}${row('Weigh-in history',N.weights().length+' days','NXT.openWeightHistory()')}${row('Strength trends','','NXT.setView(\'strength\')')}${row('Waist & body scans','','NXT.setView(\'body\')')}`}`);
  }
  function more() {
